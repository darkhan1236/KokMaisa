...
export default function LeafletMap({
  ...
  return (
    <div className={`relative ${className}`} style={{ height }}>
      ...
      {/* Legend */}
      <div className="absolute left-3 bottom-3 z-[999] bg-white/90 backdrop-blur-sm rounded-lg shadow-md px-3 py-2 border border-gray-100">
        <div className="flex items-center gap-2 text-xs text-gray-600 mb-1">
          <div className="w-4 h-3 rounded-sm" style={{ background: "#22c55e", opacity: 0.7 }} />
          <span>Ферма</span>
        </div>
        <div className="flex items-center gap-2 text-xs text-gray-600">
          <div className="w-4 h-3 rounded-sm" style={{ background: "#f59e0b", opacity: 0.7 }} />
          <span>Пастбище</span>
        </div>
        <div className="text-xs text-gray-400 mt-1 text-center">ПКМ = удалить</div>
      </div>
    </div>
  );
}